...
bool SafePathing::FindPath(robot_control::IntermediateWaypoints::Request &req, robot_control::IntermediateWaypoints::Response &res)
{
	...
	if(req.collision==1) //object on left
	{
		...
		//avoidance waypoint
		waypoint.x = req.current_x + req.collisionDistance*sin(req.current_heading + turn_angle*3.14159265/180); //turn 30 deg drive 4 meters
		waypoint.y = req.current_y + req.collisionDistance*cos(req.current_heading + turn_angle*3.14159265/180); //turn 30 deg drive 4 meters
