...
void Procedure::sendDriveAndSearch()
{
	for(int i=0; i<numWaypointsToTravel; i++)
	{
		...
		if(waypointsToTravel.at(i).searchable)
		{
			...
			execActionSrv.request.pause = false;
			execActionSrv.request.pauseUnchanged = true;
			execActionSrv.request.float1 = 0.2; // white
			execActionSrv.request.float2 = 0.2; // silver
			execActionSrv.request.float3 = 0.2; // blueOrPurple
			execActionSrv.request.float4 = 0.07; // pink
			execActionSrv.request.float5 = 0.07; // red
			execActionSrv.request.float6 = 0.07; // orange
			execActionSrv.request.float7 = 0.07; // yellow
